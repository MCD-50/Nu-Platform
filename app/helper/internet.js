...
export const etherRPC = axios.create({
	baseURL: constant.config.blockchainNode.url,
	timeout: 60000,
	headers: {
...
export const platform = axios.create({
	baseURL: constant.config.platform.notifyUrl,
	timeout: 60000,
	headers: {
		"Content-Type": "application/json",
		"Accept": "application/json",
...
export const makeNodeRequest = (params, jsonMethod, id = constant.config.blockchainNode.chainId, jsonrpc = "2.0") => {
	return new Promise(async (resolve, reject) => {
		let options = {
			jsonrpc: jsonrpc,
			method: jsonMethod,
