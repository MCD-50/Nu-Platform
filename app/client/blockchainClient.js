...
export const getGasValue = (web3Client, payload) => {
	return new Promise(async (resolve, reject) => {
		...
		if (gasLimit && gasPrice) {
			gasLimit = Math.ceil(Number(web3Client.utils.hexToNumberString(gasLimit)) * constant.config.utils.gasFactor);
			gasPrice = Math.ceil(Number(web3Client.utils.hexToNumberString(gasPrice)) * constant.config.utils.gasFactor);
		}
...
const prepareEtherTransaction = (web3Client, payload, gasLimit, gasPrice, nonce) => {
	...
	if (_nonce == payload.nonce) {
		gasPriceIncrementFactor = 1.1; //Math.ceil(gasPriceIncrementFactor + (constant.config.utils.failFactor * payload.failCount));
	}
	...
	const transactionObject = Object.assign({}, {
		...
		gasPrice: web3Client.utils.toHex(Math.ceil(gasPrice * constant.config.utils.gasFactor * gasPriceIncrementFactor)),
...
const sendRawErcTransaction = (web3Client, contractInstance, payload) => {
	return new Promise(async (resolve, reject) => {
		let gasLimit = await estimateContractGas(web3Client, contractInstance, payload); //= 4712388; //await estimateContractGas(web3Client, contractInstance, payload);
		let nonce = await getTransactionCount(web3Client, payload.from);
		let gasPrice = await getGasPrice(web3Client);
